...
import { requireAuth, requireTenantId, requireRole } from "../server/middleware/authMiddleware.js";
...
export default function studentRoutes(db) {
  ...
  router.get("/me", requireAuth, requireRole("STUDENT"), requireTenantId, async (req, res) => {
    try {
      ...
      const student = await students.findOne({ userId, schoolId });
  ...
  router.get("/marks", requireAuth, requireRole("STUDENT"), requireTenantId, async (req, res) => {
    try {
      ...
      const result = await marks.find({ studentUserId, schoolId }).toArray();
      res.json(result);
    } catch (err) {
